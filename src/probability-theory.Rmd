...
Using Bayesian inference has another practical advantage: 
once you learn its simple unifying principles 
they enable you to 
[fit the model that your data deserve](https://betanalpha.github.io/assets/case_studies/modeling_and_inference.html)
instead of having to learn seemingly unrelated, ad hoc methods 
as with the frequentist approach (e.g. regularization, multiple imputation, 
robust statistics, meta-analysis, multiple comparison corrections). 
Note that this also means that with frequentist statistics the prior is often hidden in the choice of model and the procedure by which one settles on one model over the other is not reported or is not principled. Now that the computers are getting powerful enough, this is evident by the flexibility of Bayesian analysis packages such as brms which can fit very elaborate models with incrementally more effort, instead of the leaps required by someone to learn every new frequentist technique (this needs polishing)
...
##### Obstacles in adoption of Bayesian inference  
