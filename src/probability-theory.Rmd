...
The goal has traditionally been to minimise our prediction error
on future samples.
...
(e.g. regularization, multiple imputation, robust statistics, meta-analysis,
...
Opportunities to make more principled model choices
instead of culture that avoids facing shortcomings that
cannot all be addressed at the same time
and are imperceptible or, worse, swept under the rug.


Note that this also means that with frequentist statistics the prior is often hidden in the choice of model and the procedure by which one settles on one model over the other is not reported or is not principled. 

Now that the computers are getting powerful enough, this is evident by the flexibility of Bayesian analysis packages such as brms which can fit very elaborate models with incrementally more effort, instead of the leaps required by someone to learn every new frequentist technique (this needs polishing)
