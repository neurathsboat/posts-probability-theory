...
i.e. how likely is a hypothesis $H$,
which can and often does involve 
a range of parameter values instead of single values,
given the data $D$ we observed, $p(H|D)$.
This is the quantity that the Bayesian analysis is designed to estimate.
The Bayesian approach takes the observed data as definite,
...
The frequentist approach considers 
the uncertainty in the data is ireeducible, or aleatoric.
...
but it is an open question whether it can be completely eliminated.
...
up to a complete description of the wind patterns and the gravitational field,
we should be able to predict the arrow's final position with infinite precision.
However, it ultimately boils down to the question of whether
the universe is deterministic which must involve 
some discussion around the state of quantum physics,
for which I will not attempt a full exposition.
However, in practice, the Bayesian approach matches better the way scientists think:

question whether the universe is deterministic - quantum physics
that I am not prepared to attempt a full exposition.
However, in practice, the Bayesian approach matches better the way scientists think: if you care about ... then you must tell us what you knew ...
Of course, someone can choose to be "externally inconsistent" and act as if all uncertainty is aleatoric if it simplifies the calculations
